...
def apply_land_mask(
    ...
    land_mask_path: str = "data/raw/land_mask.tif",
) -> np.ndarray:
    """
    Apply land-sea mask to remove sea areas from the data array.

    Args:
        array: The data array to mask
        lon_array: Longitude coordinates for the data
        lat_array: Latitude coordinates for the data
        land_mask_path: Path to the land mask file

    Returns:
        The masked array with sea areas set to NaN
    """
...
def read_geotiff_chelsa(filepath: str, month: int) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    with rasterio.open(filepath) as src:
        ...
        array = apply_land_mask(array, lon_array, lat_array)
